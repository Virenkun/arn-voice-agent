...
class CreateToolRequest(BaseModel):
    ...
    workflow_uuid: Optional[str] = Field(
        default=None,
        description=(
            "Scope this tool to a single agent (workflow). Omit for an "
            "org-wide tool visible to every agent."
        ),
    )
...
class ToolResponse(BaseModel):
    ...
    # Agent scoping: set when the tool is visible only to one workflow/agent.
