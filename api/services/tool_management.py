...
def build_tool_response(tool: Any, include_created_by: bool = False) -> ToolResponse:
    ...
    # Agent scoping — requires the query to have eagerly loaded ToolModel.workflow
    # (selectinload); a lazy-load here would raise MissingGreenlet.
    ...
    return ToolResponse(
        ...
        workflow_uuid=workflow.workflow_uuid if workflow else None,
...
async def create_tool_for_user(
    ...
    *,
    ...
    # Optional agent scoping: resolve the workflow within the caller's org and
    # reject if it isn't theirs (the FK alone doesn't prove ownership).
    workflow_id = None
    if request.workflow_uuid:
        workflow = await db_client.get_workflow_by_uuid(
            request.workflow_uuid, user.selected_organization_id
        )
        if not workflow:
            raise ToolManagementError(
                "workflow_not_found",
