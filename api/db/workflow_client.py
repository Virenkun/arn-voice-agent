...
class WorkflowClient(BaseDBClient):
    ...
    async def get_agent_types_for_workflows(
        self, workflow_ids: list[int]
    ) -> dict[int, Optional[str]]:
        """Map workflow_id -> the `agent_type` marker (e.g. "single_prompt").

        The marker lives in a definition's `workflow_configurations`. This reads
        only the JSON key (Postgres ``->>``), not the whole blob, so it stays
        cheap for listings, and prefers the draft over the current published
        version — mirroring how the editor resolves config.
        """
        if not workflow_ids:
            return {}

        agent_type_col = WorkflowDefinitionModel.workflow_configurations[
            "agent_type"
        ].as_string()
        ...
        async with self.async_session() as session:
            ...
            draft_seen: set[int] = set()
            for workflow_id, status, agent_type in result.all():
                if status == "draft":
                    resolved[workflow_id] = agent_type
                    draft_seen.add(workflow_id)
                elif workflow_id not in draft_seen:
                    resolved[workflow_id] = agent_type
            return resolved
