...
class ToolClient(BaseDBClient):
    ...
    async def create_tool(
        ...
        Args:
            ...
            workflow_id: Optional workflow scoping (None = org-global,
                set = visible only to that workflow's agent)
        ...
        async with self.async_session() as session:
            ...
            session.add(tool)
            # Flush to assign the PK while the instance is still fresh —
            # after commit() the instance expires and attribute access would
            # lazy-load (MissingGreenlet in async context).
            await session.flush()
            tool_id = tool.id
            await session.commit()

            # Re-select with the workflow relationship eagerly loaded — callers
            # build ToolResponse (workflow_uuid/name) outside this session, and
            # an async lazy-load there would raise MissingGreenlet.
            result = await session.execute(
                select(ToolModel)
                .where(ToolModel.id == tool_id)
                .options(selectinload(ToolModel.workflow))
            )
            ...
            logger.info(
                ...
                + (f" scoped to workflow {workflow_id}" if workflow_id else "")
            )
            return tool
    ...
    async def get_tools_for_organization(
        ...
        Args:
            ...
            workflow_id: When set, return org-global tools plus tools scoped to
                this workflow (an agent's picker view). When None, return
                everything including agent-scoped tools (the /tools page view).
        ...
        async with self.async_session() as session:
            query = (
                select(ToolModel)
                .where(ToolModel.organization_id == organization_id)
                .options(selectinload(ToolModel.workflow))
            )
            ...
            if workflow_id is not None:
                query = query.where(
                    or_(
                        ToolModel.workflow_id.is_(None),
                        ToolModel.workflow_id == workflow_id,
                    )
                )
    ...
    async def get_tool_by_uuid(
        ...
        async with self.async_session() as session:
            query = (
                ...
                .options(
                    selectinload(ToolModel.created_by_user),
                    selectinload(ToolModel.workflow),
                )
            )
