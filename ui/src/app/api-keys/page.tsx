...
export default function APIKeysPage() {
  ...
  const [createdServiceKey, setCreatedServiceKey] =
    useState<CreateServiceKeyResponse | null>(null);
  const [showCreatedKeyDialog, setShowCreatedKeyDialog] = useState(false);
  const [showCreatedServiceKeyDialog, setShowCreatedServiceKeyDialog] =
    useState(false);
  ...
  const fetchApiKeys = useCallback(async () => {
    ...
    if (loading || !user) {
      logger.debug(
        "[APIKeysPage] fetchApiKeys - skipping due to loading or no user",
      );
      return;
    }
  ...
  const fetchServiceKeys = useCallback(async () => {
    ...
    if (loading || !user) {
      logger.debug(
        "[APIKeysPage] fetchServiceKeys - skipping due to loading or no user",
      );
      return;
    }
    ...
    try {
      ...
      setError(null);
      logger.debug(
        "[APIKeysPage] fetchServiceKeys - calling getAccessToken...",
      );
      const accessToken = await getAccessToken();
  ...
  return (
    <div className="min-h-[90%]">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">
              Manage your API keys to access {PRODUCT_NAME} services
              programmatically
            </p>
          </div>
          ...
          {/* Managed service keys (vendor MPS) — hidden in OSS / BYOK builds */}
          {!isOSS && (
            <Card className="mb-6">
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <CardDescription>
                      Manage service keys for accessing Dograh AI services (LLM,
                      TTS, STT)
                    </CardDescription>
                  </div>
                  <div className="flex gap-2">
                    {showServiceKeyArchiveControls && (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setShowServiceArchived(!showServiceArchived)
                        }
                      >
                        {showServiceArchived ? (
                          <Eye className="w-4 h-4 mr-2" />
                    ...
                    {canCreateServiceKey ? (
                      ...
                      <span className="text">
                        To generate additional service keys,{" "}
                        <a
                          ...
                          {!key.archived_at &&
                            showServiceKeyArchiveControls && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleArchiveServiceKey(String(key.id))
                                }
                                className="text-destructive hover:text-destructive/90"
                              >
                                <Trash2 className="w-4 h-4" />
          ...
          <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            <p className="text-sm text-yellow-600 dark:text-yellow-500">
              <strong>Important:</strong> Keep your API keys secure. Never share
              them publicly or commit them to version control. API keys provide
              full access to your organization&apos;s resources.
            </p>
          </div>
        </div>
      </div>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a descriptive name for your API key to help you identify it
              later.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent>
          ...
          {createdKey && (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                <p className="text-sm text-yellow-600 dark:text-yellow-500">
                  Store this key securely. It will only be shown once and cannot
                  be retrieved later.
                </p>
              </div>
            </div>
      ...
      <Dialog
        open={showCreatedServiceKeyDialog}
        onOpenChange={setShowCreatedServiceKeyDialog}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Service Key Created Successfully</DialogTitle>
            <DialogDescription>
              Make sure to copy your service key now. You won&apos;t be able to
              see it again!
            </DialogDescription>
          </DialogHeader>
          {createdServiceKey && (
            <div className="space-y-4">
              <div className="p-4 bg-muted rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      copyToClipboard(createdServiceKey.service_key)
                    }
                  >
                    <Copy className="w-4 h-4" />
                  </Button>
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <p className="text-sm text-blue-600 dark:text-blue-500">
                  This key provides access to Dograh AI services including LLM,
