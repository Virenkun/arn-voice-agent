...
import { SinglePromptEditor } from '@/components/workflow/single-prompt/SinglePromptEditor';
...
function RenderWorkflow({
    ...
    // Single-prompt agents render a simplified one-prompt editor instead of the
    // canvas. Derive from the prop (synchronous) to avoid a canvas flash before
    // the store hydrates, then track the live store value once available.
    const isSinglePrompt =
        (workflowConfigurations ?? initialWorkflowConfigurations)?.agent_type === 'single_prompt';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch tools — scoped to this workflow so the picker shows
                // org-global tools plus tools created for this agent only.
                const toolsResponse = await listToolsApiV1ToolsGet(
                    workflowUuid ? { query: { workflow_uuid: workflowUuid } } : {},
                );
    ...
    const onToolCreated = useCallback((tool: ToolResponse) => {
        setTools((prev) => (prev ? [...prev, tool] : [tool]));
    }, []);
                ...
                <div className="flex-1 min-h-0">
                    <div className="flex h-full min-w-0">
                        <div className="relative min-w-0 flex-1">
                            {isSinglePrompt ? (
                              <>
                                <SinglePromptEditor workflowId={workflowId} />
                                {/* Hidden 1px ReactFlow so saveWorkflow()'s rfInstance guard +
                                    getViewport() still work without a visible canvas (R1). */}
                                <div
                                    aria-hidden
