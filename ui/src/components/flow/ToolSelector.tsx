...
import { AgentToolSheet, type AgentToolSheetMode } from "@/components/tools/AgentToolSheet";
...
export function ToolSelector({
    ...
    const scopeUuid = workflow?.workflowUuid;
    // Belt-and-braces: drop tools scoped to a DIFFERENT agent even if the
    // fetch was unfiltered (the list API already filters when scoped).
    const activeTools = tools.filter(
        (t) =>
            t.status === "active" &&
            (!t.workflow_uuid || !scopeUuid || t.workflow_uuid === scopeUuid),
    );
    ...
    // Agent-scoped tool creation/editing (available inside an agent editor).
    const canCreateAgentTool = !!(scopeUuid && workflow?.onToolCreated);
    const [sheetOpen, setSheetOpen] = useState(false);
    const [sheetMode, setSheetMode] = useState<AgentToolSheetMode>({ kind: "create" });

    const openCreateSheet = () => {
        setSheetMode({ kind: "create" });
        setSheetOpen(true);
    };
    const openEditSheet = (tool: ToolResponse) => {
        setSheetMode({ kind: "edit", tool });
        setSheetOpen(true);
    };

    const handleSheetSaved = (tool: ToolResponse) => {
        if (sheetMode.kind === "create") {
            workflow?.onToolCreated?.(tool);
            // Auto-attach non-MCP tools; MCP tools attach via function selection
            // once their catalog is discovered.
            if (tool.category !== "mcp" && !value.includes(tool.tool_uuid)) {
                onChange([...value, tool.tool_uuid]);
            }
        } else {
            workflow?.updateTool?.(tool.tool_uuid, () => tool);
        }
    };

    const agentBadgeFor = (tool: ToolResponse) =>
        tool.workflow_uuid && tool.workflow_uuid === scopeUuid ? (
            <Badge
                variant="secondary"
                className="shrink-0 border-transparent bg-violet-500/15 text-[10px] text-violet-700 dark:text-violet-300"
            >
                This agent
            </Badge>
        ) : null;
    ...
    return (
        <div className="grid gap-2">
            ...
            {activeTools.length === 0 ? (
                <div className="p-4 border rounded-md text-center">
                    ...
                    <div className="flex items-center justify-center gap-2">
                        {canCreateAgentTool && (
                            <Button variant="default" size="sm" onClick={openCreateSheet}>
                ...
                <Tabs defaultValue="http">
                            ...
                            {httpTools.map((tool) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {canCreateAgentTool && tool.workflow_uuid === scopeUuid && (
                                            <Button
                                                type="button"
                                                variant="ghost"
                                                size="icon"
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2 p-2 bg-muted/30 rounded-md">
                        <Link
                            href="/tools"
                            target="_blank"
            ...
            {canCreateAgentTool && scopeUuid && (
                <AgentToolSheet
                    open={sheetOpen}
                    onOpenChange={setSheetOpen}
                    workflowUuid={scopeUuid}
                    mode={sheetMode}
